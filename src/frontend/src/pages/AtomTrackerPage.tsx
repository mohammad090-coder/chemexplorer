...
interface OrbitalStep {
  subshell: string; // "1s", "2p", etc.
  shell: number; // n = 1, 2, 3 …
  electrons: number; // electrons in this subshell
  maxElectrons: number;
  cumulative: number; // total electrons so far after this step
  blockType: "s" | "p" | "d" | "f";
}
...
function parseConfig(raw: string): OrbitalStep[] {
  // Normalize superscript unicode → ASCII digits
  const normalized = normalizeSuperscripts(raw);
  // Match tokens like "1s2", "2p6", "[Xe]" (skip noble-gas shortcuts)
  const tokenRe = /(\d+)([spdf])(\d+)/g;
  const steps: OrbitalStep[] = [];
  let cumulative = 0;
  let match = tokenRe.exec(normalized);
  while (match !== null) {
    const shell = Number.parseInt(match[1], 10);
    const blockType = match[2] as "s" | "p" | "d" | "f";
...
function getValenceElectrons(steps: OrbitalStep[]): number {
  if (steps.length === 0) return 0;
  const maxShell = Math.max(...steps.map((s) => s.shell));
  // d-block: valence = outermost s + d electrons at highest shell/subshell
  return steps
    .filter((s) => s.shell === maxShell)
    .reduce((sum, s) => sum + s.electrons, 0);
}

function getBlock(steps: OrbitalStep[]): string {
  if (steps.length === 0) return "—";
  const last = steps[steps.length - 1];
  return last.blockType.toUpperCase();
}

function isParamagnetic(steps: OrbitalStep[]): boolean {
  // Check if any subshell has unpaired electrons (electrons < maxElectrons && electrons > 0)
  ...
  return (
    <span
      ...
      style={{
        ...
        background: filled
          ? "radial-gradient(circle at 35% 35%, oklch(0.92 0.18 200), oklch(0.65 0.22 205))"
          : "transparent",
        borderColor: filled
          ? "oklch(0.72 0.22 200 / 0.8)"
          : "oklch(0.45 0.06 250 / 0.5)",
        boxShadow: filled
          ? `0 0 8px oklch(0.72 0.22 200 / ${isActive ? "0.9" : "0.4"})`
          : "none",
...
function OrbitalRow({
  ...
  step: OrbitalStep;
  revealedElectrons: number;
  isCurrentShell: boolean;
  isCompleted: boolean;
  isActive: boolean;
}) {
  ...
  return (
    <div
      className={[
        ...
        isCurrentShell && !isCompleted
          ? "border-cyan-400/40 bg-cyan-500/8 shadow-[0_0_20px_oklch(0.72_0.22_200/0.12)]"
          : isCompleted
            ? "border-border/30 bg-white/3 opacity-70"
            : "border-border/20 bg-white/2 opacity-40",
      ].join(" ")}
    >
      {/* Subshell label */}
      <span
      ...
      <span className="font-mono text-xs text-muted-foreground shrink-0 w-12 text-right">
        {revealedElectrons}/{step.maxElectrons}
      </span>

      {/* Cumulative */}
      {revealedElectrons === step.electrons && (
        <span className="text-[10px] text-cyan-400/70 font-mono shrink-0 w-12 text-right">
          Σ {step.cumulative}
        </span>
      )}
    </div>
...
export function AtomTrackerPage() {
  const [query, setQuery] = useState("");
  const [selectedElement, setSelectedElement] = useState<Element>(ELEMENTS[0]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [currentStep, setCurrentStep] = useState(-1); // -1 = reset state
  ...
  const prevSymbolRef = useRef<string>(selectedElement.symbol);
    ...
    query.trim().length > 0
      ? ELEMENTS.filter(
          (el) =>
            ...
            String(el.atomicNumber).startsWith(query),
        ).slice(0, 8)
      : [];

  // Reset when element changes (avoid stale-closure dep warning)
  ...
  // Determine revealed state per step
  function getRevealedElectrons(stepIdx: number): number {
    if (currentStep < stepIdx) return 0;
    if (currentStep > stepIdx) return steps[stepIdx].electrons;
    return steps[stepIdx].electrons; // fully reveal current step at once
  }
  ...
  const totalRevealedElectrons =
    currentStep >= 0 ? steps[currentStep].cumulative : 0;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-muted-foreground text-sm md:text-base">
            Visualize electron configuration filling step-by-step
          </p>
        </div>
        ...
        >
          ...
          <div className="relative" ref={inputWrapperRef}>
            ...
            {/* Suggestions dropdown — floats above ALL UI layers */}
            <AnimatePresence>
        ...
        >
          <div className="flex items-center justify-between w-full">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
              Atom Preview — {selectedElement.name}
            </p>
            {/* Model toggle */}
            <div
            ...
            >
              {(["bohr", "orbital"] as AtomModel[]).map((m) => (
                ...
                >
                  {m === "bohr" ? "Bohr Model" : "Orbital Model"}
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            ...
            style={{
              background: isPlaying
                ? "oklch(0.65 0.18 22 / 0.2)"
                : "oklch(0.72 0.22 200 / 0.2)",
              border: isPlaying
                ? "1px solid oklch(0.65 0.18 22 / 0.5)"
                : "1px solid oklch(0.72 0.22 200 / 0.5)",
              color: isPlaying ? "oklch(0.8 0.2 22)" : "oklch(0.88 0.16 200)",
              boxShadow: isPlaying
                ? "0 0 16px oklch(0.65 0.18 22 / 0.2)"
                : "0 0 16px oklch(0.72 0.22 200 / 0.2)",
            }}
        ...
        <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
          <div
            ...
            style={{
              ...
              background:
                "linear-gradient(90deg, oklch(0.72 0.22 200), oklch(0.68 0.2 260))",
        ...
        >
          ...
          {steps.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-6">
              Could not parse electron configuration for this element.
            </p>
          ) : (
            <div className="space-y-3">
              {shellGroups.map((shell) => {
                ...
                return (
                  <div key={shell} className="space-y-1.5">
                    ...
                    {/* Orbital rows for this shell */}
                    <div className="space-y-1 pl-2">
                      {shellSteps.map((step, localIdx) => {
          ...
          >
            {currentStep < 0 ? (
              <span className="text-muted-foreground/50">
                Press Play or Next to begin filling orbitals…
              </span>
            ) : (
              steps.slice(0, currentStep + 1).map((s, i) => (
                <span key={s.subshell}>
                  <span className={BLOCK_COLOR[s.blockType]}>
                    {s.subshell}
                    <sup>{s.electrons}</sup>
                  </span>
                  {i < currentStep ? " " : ""}
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Atomic Number {selectedElement.atomicNumber} · Period{" "}
                {selectedElement.period} · Group {selectedElement.group || "—"}
              </div>
            </div>
            <div className="ml-auto text-right">
              <div className="text-xs text-muted-foreground">Config</div>
              <div className="text-xs font-mono text-cyan-300/80 max-w-[140px] truncate">
                {selectedElement.electronConfiguration}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
            {(["s", "p", "d", "f"] as const).map((b) => (
              <span key={b} className="flex items-center gap-1.5">
                <span className={`w-3 h-3 rounded-sm border ${BLOCK_BG[b]}`} />
                <span className={BLOCK_COLOR[b]}>{b.toUpperCase()}-block</span>
              </span>
            ))}
            <span className="flex items-center gap-1.5">
              <span
                ...
                style={{
                  background:
                    "radial-gradient(circle, oklch(0.88 0.18 200), oklch(0.65 0.22 205))",
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full border border-muted-foreground/30 opacity-40" />
              Empty slot
            </span>
          </div>
        </div>
