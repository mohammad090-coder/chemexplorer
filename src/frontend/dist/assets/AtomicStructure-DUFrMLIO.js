import { r as reactExports, y as useAnimationLevel, j as jsxRuntimeExports, m as motion } from "./index-DyyHqAHL.js";
...
function parseOrbitalToken(token) {
...
function parseShells(electronConfiguration) {
  ...
  const tokens = rest.match(/\d[spdf]\d+/g) ?? rest.replace(/\s+/g, "").match(/\d[spdf]\d+/g) ?? [];
...
function parseShellSubshells(electronConfiguration) {
  const normalized = normalizeSuperscripts(electronConfiguration.trim());
  let rest = normalized;
  const nobleMatch = rest.match(/^\[([A-Z][a-z]?)\]\s*/);
...
const SHELL_DURATIONS = [2e3, 3200, 4800, 7e3, 1e4];
...
const AtomicStructure = reactExports.memo(function AtomicStructure2({
  atomicNumber,
  ...
  const allShells = reactExports.useMemo(
    () => parseShells(electronConfiguration),
    [electronConfiguration]
  );
  const maxShells = animationLevel === "minimal" ? 2 : animationLevel === "reduced" ? 3 : 5;
  const shells = reactExports.useMemo(
    () => allShells.slice(0, maxShells),
    [allShells, maxShells]
  );
  const shellSubshells = reactExports.useMemo(
  ...
  const [rotX, setRotX] = reactExports.useState(0);
  const [rotY, setRotY] = reactExports.useState(0);
  ...
  const getOrbitalLabel = reactExports.useCallback(
    (shellIdx, eIdx) => {
      const n = shellIdx + 1;
      ...
      for (const sub of subs) {
        ...
        if (eIdx < cumCount + cap) {
          const ml = sub === "s" ? 0 : Math.floor((eIdx - cumCount) / 2) - Math.floor(cap / 4);
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
              ...
              children: [
                ...
                shells.slice(0, 2).map((electronCount, shellIdx) => {
                  ...
                  return Array.from({ length: electronCount }).map((_item, eIdx) => {
                    ...
                    const dur = SHELL_DURATIONS[shellIdx] / 1e3;
          ...
          showToggle && onModelChange && /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              ...
              children: ["bohr", "orbital"].map((m) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  style: {
                    ...
                    borderRight: m === "bohr" ? `1px solid ${electronColor}22` : "none"
                  },
  ...
  const nucleusParticles = animationLevel === "reduced" ? NUCLEUS_PARTICLES_MINIMAL : NUCLEUS_PARTICLES;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    motion.div,
    {
      className: "flex flex-col items-center gap-3 w-full select-none",
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          ...
          {
            ref: containerRef,
            className: animationLevel === "full" ? "relative shrink-0 cursor-grab active:cursor-grabbing" : "relative shrink-0",
            style: {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                style: {
                  ...
                  transition: dragRef.current ? "none" : void 0
                },
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "svg",
                    {
                      ...
                      children: [
                        ...
                        model === "orbital" && animationLevel === "full" && shells.map((_, shellIdx) => {
                          const n = shellIdx + 1;
                          const subs = shellSubshells[n] ?? [];
                          if (!subs.includes("p")) return null;
                          const r = SHELL_RADII[shellIdx];
                          return /* @__PURE__ */ jsxRuntimeExports.jsxs("g", { opacity: 0.14, children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx(
                              "ellipse",
                              {
                                cx: center - r * 0.7,
                        ...
                        model === "bohr" && shells.map((_, shellIdx) => {
                          const r = SHELL_RADII[shellIdx];
                          const n = shellIdx + 1;
                          return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "text",
                  ...
                  nucleusParticles.map((particle) => {
                    const [ox, oy, sz, isProton] = particle;
                    ...
                    return /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        style: {
                          ...
                          zIndex: isCenter ? 5 : 3,
                          willChange: "transform",
                          animationName: isCenter ? `${pulseAnim}, ${pulseScaleAnim}` : void 0,
                          animationDuration: isCenter ? "2.4s, 2.4s" : void 0,
                          animationTimingFunction: isCenter ? "ease-in-out, ease-in-out" : void 0,
                          animationIterationCount: isCenter ? "infinite, infinite" : void 0
                        },
                        "aria-hidden": "true"
                      },
                      particleKey
...
const ElectronOrbit = reactExports.memo(function ElectronOrbit2({
  center,
  rx,
  ry,
  tiltDeg,
  ...
  const startTimeRef = reactExports.useRef(-1);
  ...
  reactExports.useEffect(() => {
    ...
    const animate = (ts) => {
      ...
      if (divRef.current) {
        ...
        divRef.current.style.filter = blur > 0.05 ? `blur(${blur.toFixed(2)}px)` : "";
