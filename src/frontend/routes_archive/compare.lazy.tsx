...
const COMPARE_PROPS: Array<{
  ...
  },
  {
    key: "category",
    label: "Category",
    fmt: (v) => CATEGORY_LABELS[v as keyof typeof CATEGORY_LABELS] ?? String(v),
  },
...
function ElementPicker({
  ...
  if (selected) {
    return (
      <div
        className={cn(
          "relative glass rounded-2xl p-6 text-center overflow-hidden",
        ...
        <div className="space-y-1">
          {results.map((el) => (
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 p-2 rounded-lg text-left hover:bg-card/60 transition-all",
              )}
